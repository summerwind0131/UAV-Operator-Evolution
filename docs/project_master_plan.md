...
双向三臂 lifecycle smoke 已完成：同一 master seed 下，UAV/JSSP 均运行 scratch、same-domain、cross-domain 三臂；每臂 64 calls、1 个 validation instance、1 个候选。same/cross 固定检索 top-4，目标域重新生成并通过自己的 IR/compiler smoke gate；六个 outcome 均未打开 test，远程调用数为 0。receipt 为 `artifacts/releases/mechanism-transfer-v1.arms-smoke.json`，payload SHA-256 `f023f6ba…f0f92b`。
