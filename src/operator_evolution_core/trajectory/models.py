"""Domain-independent trace models for complete operator decisions."""
...
class OperatorTrace(BaseModel):
    ...
    # ``map_id`` remains the serialized/SQLite v1 compatibility name. New core
    # callers may supply ``instance_id`` and use the property below.
    map_id: str = Field(
        default="",
        validation_alias=AliasChoices("map_id", "instance_id"),
    )
    ...
    def instance_id(self) -> str:
        """Domain-independent identity, backed by the v1 ``map_id`` field."""
