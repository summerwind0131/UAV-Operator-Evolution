...
class OperatorDiagnoser:
    ...
    def __init__(
        ...
        *,
        ...
        self.group_by = self._resolve_group_by(group_by)
    ...
    def _resolve_group_by(
        self, group_by: Sequence[str] | str | None
    ) -> tuple[str, ...]:
        fields = self._normalise_group_by(group_by)
        if self.feature_catalog is None:
            return fields
        return tuple(self.feature_catalog.resolve(name) for name in fields)
    ...
    def diagnose(
        ...
        *,
        ...
        fields = self.group_by if group_by is None else self._resolve_group_by(group_by)
    ...
    def analyze_synergies(
        ...
        *,
        ...
        return compute_sequential_synergies(
            ...
            max_gap=max_gap,
            group_by=(
                self.group_by if group_by is None else self._resolve_group_by(group_by)
            ),
...
def compute_sequential_synergies(
    ...
    *,
    ...
    for trace in items:
        trajectories[(trace.run_id, trace.episode_id, trace.instance_id)].append(trace)
