...
def decide_retention(
    ...
    *,
    ...
    hard = [outcome.gain for outcome in outcomes if outcome.difficulty in {"dense", "corridor"}]
    ...
    runtime_gate = runtime_effect_gate and runtime_evidence_eligible
    reasons: list[str] = []
    if global_gate:
    ...
    if not safety_passed:
        ...
        if runtime_effect_gate and not runtime_evidence_eligible:
...
def validate_operator_contract(
    ...
    """Exercise runtime-generated operators against structural safety invariants."""
    ...
    for seed in seeds:
        ...
        try:
            result = operator.apply(list(path), environment, np.random.default_rng(seed), context)
        except Exception as exc:  # safety gate reports rather than propagates
        ...
        if elapsed_ms > max(deadline_ms * 3.0, deadline_ms + 10.0):
            failures.append(f"seed {seed}: exceeded contract runtime ({elapsed_ms:.3f} ms)")
    return failures
